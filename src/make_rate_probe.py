...
def main() -> None:
    data = np.load(ART / "probs_test_reproduced.npz", allow_pickle=True)
    test = pd.read_csv(ROOT / "test.csv")
    assert (data["claim_id"].astype(str) == test["claim_id"].astype(str).to_numpy()).all()
    scores = np.mean([data[c] for c in COMPONENTS], axis=0)
    ...
    for tag, multiplier in MULTIPLIERS.items():
        ...
        path = SUBS / f"submission_equal4_{tag}_fp.csv"
